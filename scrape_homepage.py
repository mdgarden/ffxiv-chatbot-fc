...
def extract_maintenance_post_jp():
    jp_notice_block = get_soup(FFXIV_JP_URL+LODESTONE).find("div", {"class" : "toptabchanger_newsbox"})
    ...
    maintenance_contents = get_soup(FFXIV_JP_URL+maintenance_url).find("div", {"class":"news__detail__wrapper"}).get_text()
...
def extract_character_profile(info):
    character_number = info['profile']
    return FFXIV_JP_URL + LODESTONE + CHARACTER + "/" + character_number
